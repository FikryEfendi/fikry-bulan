...
async function kirimUcapan() {
  ...
  try {
    ...
    const response = await fetch('/api/tamu/add', {
      method: 'POST',
...
function addUcapanToList(nama, pesan, status, prepend = true) {
  ...
  const badgeClass = status === 'hadir' ? 'hadir-badge' : status === 'tidak' ? 'tidak-badge' : 'belum-badge';
  const badgeText  = status === 'hadir' ? '✓ Hadir' : status === 'tidak' ? '✗ Tidak Hadir' : '-';
  el.innerHTML = `
